...
describe.sequential("database integration", () => {
  ...
  it("updates prompt metadata and rejects unknown prompts", async () => {
    const created = await createPrompt(postgres.db, projectId, "integration-user", {
      name: `integration/update-${crypto.randomUUID()}`,
      description: "Before",
      content: { type: "text", template: "Hello {{name}}" },
    });
    try {
      expect(
        await updatePrompt(postgres.db, projectId, created.id, { description: "After" }),
      ).toMatchObject({ description: "After", versionCount: 1 });
      expect(await getPrompt(postgres.db, projectId, created.id)).toMatchObject({
        description: "After",
      });
      expect(
        await updatePrompt(postgres.db, projectId, crypto.randomUUID(), { description: "Missing" }),
      ).toBeUndefined();
    } finally {
      await postgres.sql`DELETE FROM prompts WHERE id = ${created.id}`;
    }
  });
  ...
  it("warns only when compared runs differ in dataset or prompt identity", async () => {
    await insertEvaluationRuns(clickhouse, [
      evaluationRun(),
      {
        ...
        promptVersion: "4",
        ingestVersion: (BigInt(unixNano()) + 3n).toString(),
      },
      {
        ...evaluationRun(),
        id: "run-same-prompt",
        ingestVersion: (BigInt(unixNano()) + 4n).toString(),
      },
    ]);

    const differing = await compareEvaluationRuns(
      clickhouse,
      projectId,
      "run-1",
      "run-other-prompt",
    );
    expect(differing?.warnings).toEqual(
      expect.arrayContaining(["Runs use different prompts", "Runs use different prompt versions"]),
    );

    const matching = await compareEvaluationRuns(clickhouse, projectId, "run-1", "run-same-prompt");
    expect(matching?.warnings).not.toContain("Runs use different prompts");
    expect(matching?.warnings).not.toContain("Runs use different prompt versions");

    expect(
      await compareEvaluationRuns(clickhouse, projectId, "run-1", crypto.randomUUID()),
    ).toBeUndefined();
  });
