...
export const createMcpTokensRouter = (deps: ApiDependencies) =>
  new Hono<AppEnv>()
    ...
    .post("/:projectId/mcp-tokens", async (c) => {
      ...
      if (access === undefined) return apiError(c, 404, "not_found", "Project not found");
      if (!canManage(access.role)) return adminRequired(c);
      const parsed = createMcpTokenSchema.safeParse(await safeJson(c));
      if (!parsed.success) return apiError(c, 400, "invalid_token", "Invalid MCP token data");
      const expiresAt = parsed.data.expiresAt === null ? null : new Date(parsed.data.expiresAt);
      ...
      const [created] = await deps.postgres.db
        ...
        .values({
          ...
          allowRawPayloads: parsed.data.allowRawPayloads,
